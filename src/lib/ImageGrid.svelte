<script lang="ts">
	...
	if (images.length > 0) alerts.push({snippet: alertError, text: "Error! No sexy images found."})

	let editModal: {open: boolean, name: string, value: string} = $state({open: false, name: "", value: ""})
	...
	let deleteModal: {open: boolean, name: string} = $state({open: true, name: "tester"})
	...
	const openDeleteModal = (props: Image) => deleteModal = {
		open: true,
		name: props.name,
	};
</script>
	...
	<div class="p-3 m-2 rounded-xl shadow-sm hover:shadow-xl hover:-translate-y-0.5 hover:cursor-pointer transition bg-base-300 relative" onclick={_ => goto(`/manage/${props.name}`)}>
		...
		<div class="absolute right-0 top-0 m-3 dropdown dropdown-bottom dropdown-center" style="z-index: 50" onclick={e => e.stopPropagation()}>
			...
			<ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box p-2 shadow-sm">
				...
				<!-- svelte-ignore a11y_consider_explicit_label -->
				<li><button class="btn btn-square" onclick={_ => openDeleteModal(props)}><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M6 19a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V7H6zm2.46-7.12l1.41-1.41L12 12.59l2.12-2.12l1.41 1.41L13.41 14l2.12 2.12l-1.41 1.41L12 15.41l-2.12 2.12l-1.41-1.41L10.59 14zM15.5 4l-1-1h-5l-1 1H5v2h14V4z"/></svg></button></li>
			</ul>
		</div>
		<img
...
{#if images.length > 0}
	<div class="w-full h-screen flex justify-center items-center">
		<Masonary item={imageItem} items={images} preloadItem={preload} columnWidth="w-3xs sm:w-xs lg:w-md xl:w-lg" bind:update={update}/>
	</div>
{/if}
...
<dialog class="modal" open={deleteModal.open}>
	<div class="modal-box">
		<h3 class="text-lg font-bold">Would you like to delete <i>{deleteModal.name}</i>?</h3>
